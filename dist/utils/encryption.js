 ...
 */
export const generateKeyFromPassword = async (password, salt = 'defaultSalt', options = DEFAULT_ENCRYPTION_OPTIONS) => {
    try {
 ...
 */
export const encryptText = async (text, keyString, options = DEFAULT_ENCRYPTION_OPTIONS) => {
    try {
        if (!window.crypto || !window.crypto.subtle) {
            throw new Error('Web Crypto API not available');
        }
 ...
 */
export const decryptText = async (encryptedText, keyString, options = DEFAULT_ENCRYPTION_OPTIONS) => {
    try {
        if (!window.crypto || !window.crypto.subtle) {
            throw new Error('Web Crypto API not available');
        }
 ...
 */
export const testEncryption = async (key, options = DEFAULT_ENCRYPTION_OPTIONS) => {
    try {
 ...
 */
export const createEncryptionFunctions = (key, options = DEFAULT_ENCRYPTION_OPTIONS) => {
    return {
        generateKeyFunctionProp: async (password) => generateKeyFromPassword(password, 'defaultSalt', options),
        encryptFunctionProp: async (text) => encryptText(text, key, options),
        decryptFunctionProp: async (text) => decryptText(text, key, options),
    };
};
/**
 * Check encryption status
 */
export const getEncryptionStatus = async (key, options = DEFAULT_ENCRYPTION_OPTIONS) => {
