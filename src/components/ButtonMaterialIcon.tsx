...
export const ButtonMaterialIcon = React.forwardRef<HTMLButtonElement, ButtonMaterialIconProps>(
  ({ icon, size = 20, rounded = 8, className = '', background = 'var(--btn-bg, #eef2f7)', style, ...rest }, ref) => {
    return (
      <button
        ref={ref}
      ...
      >
        <span className="mi" style={{ fontSize: size }}>{icon}</span>
      </button>
    );
  }
);
